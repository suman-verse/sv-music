...
import 'package:sv_music/models/position_data.dart';
...
import 'package:sv_music/utilities/map_utils.dart';
import 'package:sv_music/utilities/mediaitem.dart';
import 'package:sv_music/utilities/queue_entry_utils.dart';
import 'package:rxdart/rxdart.dart';

class SvMusicAudioHandler extends BaseAudioHandler {
  SvMusicAudioHandler() {
  ...
  MediaItem? _mediaItemForResumption(Map song) {
    ...
    return mapToMediaItem(normalisedSong).copyWith(
      ...
      displaySubtitle: artist.isEmpty ? 'SV Music' : artist,
    );
  }
  ...
  /// Starts an endless radio station based on the vibe of [seedSong].
  Future<void> startRadio(Map seedSong) async {
    try {
      if (offlineMode.value) return;

      final seedYtId = seedSong['ytid']?.toString();
      if (seedYtId == null || seedYtId.isEmpty) return;

      // 1. Play the seed song immediately
      await addPlaylistToQueue([seedSong], replace: true, startIndex: 0);

      // 2. Fetch radio station tracks matching seed
      unawaited(
        Future.microtask(() async {
          try {
            final radioTracks = await getSongRadio(seedYtId, limit: 30);
            if (radioTracks.isNotEmpty) {
              final filtered = radioTracks.where((track) {
                return track['ytid']?.toString() != seedYtId;
              }).toList();

              for (final track in filtered) {
                final queueSong = _queueEntryIds.createSong(track);
                queueSong['isAutoPicked'] = true;
                _queueList.add(queueSong);
              }
              _updateQueueMediaItems();
            ...
            logger.log(
              'Error populating radio station',
              error: e,
              stackTrace: stackTrace,
            );
          }
      ...
      logger.log('Error starting radio', error: e, stackTrace: stackTrace);
    }
  }
