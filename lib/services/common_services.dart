...
Future<List> _getRecommendationsFromRecentlyPlayed() async {
  try {
    final recent = List.from(userRecentlyPlayed.value);
    if (recent.isEmpty) {
      return await _getRecommendationsFromMixedSources();
    }

    // Take up to 3 most recent seeds
    final seeds = recent.take(3).toList();
    ...
    logger.log(
      'Error in _getRecommendationsFromRecentlyPlayed',
      error: e,
      stackTrace: st,
    );
  }
...
Future<void> getSimilarSong(String songYtId) async {
  try {
    final radioTracks = await getSongRadio(songYtId, limit: 10);
    if (radioTracks.isNotEmpty) {
      final candidate = radioTracks.firstWhere(
        (track) => track['ytid'] != songYtId,
        orElse: () => radioTracks.first,
      );
      nextRecommendedSong = candidate;
      return;
    }
