...
class MusicClient {
  ...
  /// Fetches an automix / radio station playlist of tracks matching the vibe,
  /// genre, and acoustic profile of [videoId].
  Future<List<Map<String, dynamic>>> getRadioTracks(
    String videoId, {
    int limit = 25,
  }) async {
    ...
    try {
      final root = await _httpClient.sendPost('next', {
        ...
        'playlistId': 'RDAMVM$normalizedVideoId',
        'isAudioOnly': true,
      });

      final tabs = root
          .getMap('contents')
          ?.getMap('singleColumnMusicWatchNextResultsRenderer')
          ?.getMap('tabbedRenderer')
          ?.getMap('watchNextTabbedResultsRenderer')
          ?.getList('tabs');

      if (tabs == null || tabs.isEmpty) return [];

      final firstTab = tabs.first;
      if (firstTab is! Map) return [];

      final contents = firstTab
          .cast<String, dynamic>()
          .getMap('tabRenderer')
          ?.getMap('content')
          ?.getMap('musicQueueRenderer')
          ?.getMap('content')
          ?.getMap('playlistPanelRenderer')
          ?.getList('contents');
      ...
      for (final rawItem in contents) {
        ...
        final item = rawItem
            .cast<String, dynamic>()
            .getMap('playlistPanelVideoRenderer');
        if (item == null) continue;

        final trackId = item.getValue<String>('videoId');
        if (trackId == null || trackId.isEmpty || !seenIds.add(trackId)) {
          continue;
        }

        final titleRuns = item.getMap('title')?.getList('runs');
        ...
        final bylineRuns = item.getMap('longBylineText')?.getList('runs');
        var artist = '';
        String? artistId;
        if (bylineRuns != null && bylineRuns.isNotEmpty) {
          final firstRun = bylineRuns.first;
          if (firstRun is Map) {
            artist = firstRun['text']?.toString() ?? '';
            artistId = firstRun
                .cast<String, dynamic>()
                .getMap('navigationEndpoint')
                ?.getMap('browseEndpoint')
        ...
        final lengthStr = item
            .getMap('lengthText')
            ?.getList('runs')
            ?.whereType<Map>()
            .map((r) => r['text']?.toString() ?? '')
            .join();
        final duration = _parseDuration(lengthStr);

        final thumbnails = item.getMap('thumbnail')?.getList('thumbnails');
        String? imageUrl;
        String? lowResImageUrl;
        String? highResImageUrl;
        if (thumbnails != null && thumbnails.isNotEmpty) {
          final validThumbnails = thumbnails.whereType<Map>().toList();
          if (validThumbnails.isNotEmpty) {
            lowResImageUrl = validThumbnails.first['url']?.toString();
            highResImageUrl = validThumbnails.last['url']?.toString();
            imageUrl = highResImageUrl ?? lowResImageUrl;
          }
        }

        imageUrl ??= 'https://i.ytimg.com/vi/$trackId/hqdefault.jpg';
        lowResImageUrl ??= 'https://i.ytimg.com/vi/$trackId/default.jpg';
        highResImageUrl ??= 'https://i.ytimg.com/vi/$trackId/maxresdefault.jpg';
        ...
        tracks.add({
          ...
          'artist': artist.trim(),
          'artistId': artistId ?? '',
          'videoAuthor': artist.trim(),
          'image': imageUrl,
          'lowResImage': lowResImageUrl,
          'highResImage': highResImageUrl,
